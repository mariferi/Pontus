...
class Product {
	...
	public static ObservableList<Product> getListFromDB() {
		...
		ResultSet rs = Application.executeQueryforRS("SELECT * FROM products");
	...
	public static void add(String code, String name, BigDecimal price, String size) {
		...
		getListFromDB().add(p);
		Application.executeQueryforUpdate("INSERT INTO products ( code, name, price, size) VALUES ('" + code + "','" + name + "','" + price + "','" + size + "')");
	...
	public static void removeFromDB(Product product) {
		...
		Application.executeQueryforUpdate(" DELETE FROM products WHERE id2=" + "'" + id + "';");
